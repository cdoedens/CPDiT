...
import src_testing.prep as prep
    ...
    for year in year_list:
        for month in range(1, 2):
            ...
            try:
                ...
                # select subset of data that can be used for training, i.e. it has data
                # continuously every 10 mins for the total_length window
                valid_timesteps = prep.get_valid_timesteps(monthly_valid_times, total_length, timestep)
                helio = helio.sel(time=valid_timesteps)
                bar = bar.sel(time=valid_timesteps)
            ...
            print(f"finished date: {year}-{month:02d}")
